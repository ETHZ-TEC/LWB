...
static uint8_t            pending_sack[4 * LWB_SCHED_SACK_BUFFER_SIZE]; // note: 4 * ... because of the memory alignment and faster index calculation!
#if !LWB_SCHED_USE_XMEM
  static lwb_stream_info_t    *streams[LWB_MAX_DATA_SLOTS];     // a list of pointers to the stream info structures, for faster access (constant time vs. linear time)
  LIST(streams_list);                                           // -> lists only work for data in RAM
  MEMB(streams_memb, lwb_stream_info_t, LWB_MAX_STREAM_CNT);    // data structures to hold the stream info
#else
  ...
  MEMBX(streams_memb, sizeof(lwb_stream_info_t), LWB_MAX_STREAM_CNT);// data structures to hold the stream info
#endif // LWB_SCHED_USE_XMEM
 ...
 */
static inline void lwb_sched_del_stream(lwb_stream_info_t* stream) {
    if (stream == 0) {        
 ...
 */
void lwb_sched_del_stream(uint32_t stream_addr) {
    ...
    uint16_t      node_id;
    uint8_t       stream_id;
    xmem_read(stream_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);
    ...
    if (streams_list == stream_addr) {       // special case: it's the first element
        ...
        do {
            xmem_read(prev_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);
        ...
        xmem_read(prev_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);   // read
        stream.next = next_addr;                                // adjust the next-pointer
        xmem_write(prev_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);  // write back
    }
 ...
 */
uint8_t lwb_sched_prep_sack(void *payload) {
    if (n_pending_sack) {
...
void lwb_sched_proc_srq(const lwb_stream_req_t * req) {
#if !LWB_SCHED_USE_XMEM
    lwb_stream_info_t *stream  = 0;     
#else
    lwb_stream_info_t stream, new_stream;
    uint32_t stream_addr, prev_addr;
#endif // LWB_SCHED_USE_XMEM
    sched_stats.t_last_req = time;
       
    if (n_pending_sack >= LWB_SCHED_SACK_BUFFER_SIZE) {
    ...
    if (req->ipi > 0) { // add and remove requests are implicitly given by the ipi (i.e. ipi of 0 implies 'remove')
        ...
        if (n_streams) {
            ...
            do {
                ...
                if (req->node_id == stream.node_id && req->stream_id == stream.stream_id) {
                    ...
                    DEBUG_PRINT_VERBOSE("stream request %u.%u processed (IPI updated)", req->node_id, req->stream_id);
                    // save the changes
                    xmem_write(stream_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);
                    return;
                }
            ...
            do {
                ...
                if (req->node_id <= stream.node_id || stream.next == MEMBX_INVALID_ADDR) { 
                    if (streams_list == prev_addr) { // the element is inserted at the head of the list
                        ...
                        xmem_write(prev_addr, sizeof(lwb_stream_info_t), (uint8_t*)&stream);
                    }                
                    break;
                }            
        ...
        xmem_write(stream_addr, sizeof(lwb_stream_info_t), (uint8_t*)&new_stream);
 ...
 */
static inline uint16_t lwb_sched_adapt_period(void) {    
    if (time < (sched_stats.t_last_req + LWB_SCHED_T_NO_REQ)) {
        // we have received stream requests in the last LWB_SCHED_T_NO_REQ seconds: set the period to a low value
        return LWB_SCHED_PERIOD_MIN;
    }
    if (!data_cnt) {
        // no streams
        return LWB_SCHED_PERIOD_IDLE; 
    }
    uint16_t new_period = (uint16_t)(((uint32_t)data_ipi * LWB_MAX_DATA_SLOTS) / data_cnt);
    //new_period = (((uint32_t)data_ipi * LWB_MAX_DATA_SLOTS) / data_cnt) * n_pkts / n_slots;
    // check for saturation
    if (new_period < LWB_SCHED_PERIOD_MIN) {
        // T_opt is smaller than LWB_SCHED_PERIOD_MIN
        DEBUG_PRINT_WARNING("network saturated!");
        return LWB_SCHED_PERIOD_MIN;
    }
    // limit the period
    if (new_period > LWB_SCHED_PERIOD_MAX) {
        return LWB_SCHED_PERIOD_MAX;
    }
    return new_period;
}
 ...
 */
static inline uint8_t lwb_sched_is_stream_in_list(uint16_t node_id, uint8_t stream_id, const uint16_t* node_list, const uint8_t* stream_list, uint8_t list_len) {
    while (list_len) {
 ...
 */
uint16_t lwb_sched_compute(lwb_schedule_t * const sched, const uint8_t * const streams_to_update, uint8_t reserve_slot_host) {
    
    static uint16_t slots_tmp[LWB_MAX_DATA_SLOTS];
    ...
    lwb_stream_info_t *curr_stream = list_head(streams_list);
    while (curr_stream != NULL) {
        ...
        if (curr_stream->n_cons_missed > LWB_SCHED_STREAM_REMOVAL_THRES) {
            // too many consecutive slots without reception: delete this stream
            lwb_stream_info_t *stream_to_remove = curr_stream;
    ...
    lwb_stream_info_t curr_stream;
    while (stream_addr != MEMBX_INVALID_ADDR) {
    
        xmem_read(stream_addr, sizeof(lwb_stream_info_t), (uint8_t*)&curr_stream);

        if (lwb_sched_is_stream_in_list(curr_stream.node_id, curr_stream.stream_id, sched->slot, streams_to_update, LWB_SCHED_N_SLOTS(sched))) {
            curr_stream.n_cons_missed = 0;
            xmem_write(stream_addr, sizeof(lwb_stream_info_t), (uint8_t*)&curr_stream);     // save changes
    ...
    lwb_stream_info_t *init_stream = curr_stream;
    do {
        // assign slots for this stream, if possible
        if ((n_slots_assigned < LWB_MAX_DATA_SLOTS) && (time >= (curr_stream->ipi + curr_stream->last_assigned))) {
    ...
    // the number of slots to assign to curr_stream
            ...
            if (period == LWB_SCHED_PERIOD_MIN) {     // if saturated 
            ...
            if (to_assign > LWB_MAX_DATA_SLOTS - n_slots_assigned) {
                to_assign = LWB_MAX_DATA_SLOTS - n_slots_assigned;     // limit
            }
...
set_schedule:
    ...
    if ((time < (sched_stats.t_last_req + LWB_SCHED_T_NO_REQ)) || (time >= (sched_stats.t_last_cont + LWB_SCHED_T_NO_REQ)) || !data_cnt) {
    ...
    if ((compressed_size + LWB_SCHED_HEADER_LEN) > LWB_MAX_PACKET_LEN) {
    ...
    return compressed_size + LWB_SCHED_HEADER_LEN;
}
...
uint16_t lwb_sched_init(lwb_schedule_t* sched) {
    ...
    period = LWB_SCHED_PERIOD_IDLE;               // set the period to the minimum at the beginning
    sched->n_slots = n_slots_assigned;  // no data slots
    LWB_SCHED_SET_CONT_SLOT(sched);         // include a contention slot
    sched_stats.t_last_cont = time;
    sched_stats.t_last_req  = -LWB_SCHED_T_NO_REQ;
    sched->time = time;
    sched->period = period;
    LWB_SCHED_SET_AS_1ST(sched);            // mark as the first schedule (beginning of a round)
    
    return LWB_SCHED_HEADER_LEN;         // empty schedule, not slots allocated yet
}
