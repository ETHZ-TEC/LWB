...
static inline void write_byte_to_register(uint8_t addr, uint8_t data) {
	// wait until the radio is ready to take an instruction
	WAIT_UNTIL_READY_FOR_INSTR();
...
static inline void set_register_field(uint8_t addr, uint8_t value, uint8_t n_bits, uint8_t offset) {
	// create the bit-mask
	...
	if (value <= mask) {
		...
		write_byte_to_register(addr,
				(read_byte_from_register(addr) & (~(mask << offset))) | (value << offset));
	}
}

// read n_bytes of buffer starting from the radio register at address addr
static inline void read_data_from_register(uint8_t addr, uint8_t *buffer, uint16_t n_bytes) {
	...
	for (i = 0; i < n_bytes - 1; i++) {
		...
		// read the data (clears also flag RFDOUTIFG and initiates auto-read for the next data byte)
		buffer[i] = RF1ADOUT1B;
	}
	// wait until the radio has provided the data
	WAIT_UNTIL_DATA_IS_READY();
...
static inline void load_SmartRF_configuration(void) {
...
// NOTE: it is assumed here that a valid command strobe is passed to the function
static inline uint8_t strobe(uint8_t strobe, uint8_t rx) {
	// mark the radio status as read (to be sure we will read the new one after the strobe)
	MARK_STATUS_AS_READ();
	// wait until the radio is ready to take an instruction
	WAIT_UNTIL_READY_FOR_INSTR();

	if (IS_XT2_ENABLED() || (strobe == RF_SXOFF) || (strobe == RF_SPWD) || (strobe == RF_SWOR) ||
			(strobe == RF_SRES) || (strobe == RF_SNOP)) {
		// if XT2 is enabled or the strobe does not put the radio into an active state
