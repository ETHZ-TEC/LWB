...
SetVCoreUp(uint8_t level)
{
  ...
  if((PMMIFG & SVMHIFG) == SVMHIFG) {
    ...
    /* Clear all Flags */
    PMMIFG &=
      ~(SVMHVLRIFG | SVMHIFG | SVSMHDLYIFG | SVMLVLRIFG | SVMLIFG |
        SVSMLDLYIFG);
  ...
  /* Set also SVS highside to new level (Vcc is high enough for a Vcore
     increase) */
...
/* Disable full-performance mode to save energy */
  SVSMLCTL &=
    ~(_HAL_PMM_DISABLE_SVSL_ + _HAL_PMM_DISABLE_SVML_ + _HAL_PMM_SVSFP);
