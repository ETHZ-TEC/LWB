...
static uint16_t slots_buffer[N_SLOTS_MAX];
...
uint16_t sched_compress(uint8_t* compressed_data, uint8_t n_slots) {
    ...
    memset(compressed_data + 2, 0, N_SLOTS_MAX * 2 - 2);    // clear the output data buffer (except for the first slot!)
	...
	for (idx = 1; idx < n_slots - 1; idx++) {
		if (slots_buffer[idx + 1] - slots_buffer[idx] == d[n_runs]) {
            ...
            if (slots_buffer[idx + 1] < slots_buffer[idx]) {
                DEBUG_PRINT_WARNING("schedule compression failure, node IDs are not sorted");
            }
			if (d[n_runs] > d_max) {
...
void sched_uncompress(uint8_t* compressed_data, uint8_t n_slots) {
    ...
    if (d_bits == 0 || d_bits > 8 || l_bits == 0) {
        DEBUG_PRINT_WARNING("schedule uncompression failure, invalid d or l bits");
    }
